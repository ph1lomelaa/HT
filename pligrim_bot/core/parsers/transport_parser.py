...
def scan_transfer_after_package(data: list[list[str]], pkg_row: int) -> dict:
    """
    Просматривает строки ПОСЛЕ заголовка пакета и ДО следующего пакета.
    Возвращает:
      {
        "types": ["автобус","поезд"],               # кратко
        "lines": ["BUS вместе с Niyet", ...],       # как в таблице
        "parsed": [                                  # если нужно подробнее
          {"type":"train","route":"JED-MED","time":"10:50","raw":"TRAIN JED-MED 10:50 14 PAX with guide"},
          ...
        ]
      }
    """
