...
def load_credentials():
    # Сначала проверяем Base64 переменную
    creds_b64 = os.getenv("GOOGLE_CREDS_BASE64")
    if creds_b64:
        try:
            # Декодируем из Base64 обратно в JSON строку
            creds_json = base64.b64decode(creds_b64).decode("utf-8")
            return json.loads(creds_json)
        except Exception as e:
            print(f"❌ Ошибка декодирования Base64 ключей: {e}")
            return None

    # Если нет Base64, ищем старую переменную (на всякий случай)
...
if creds_dict:
    print("✅ Ключи успешно загружены из переменных окружения")
    creds = Credentials.from_service_account_info(creds_dict, scopes=SCOPES)
else:
    print("❌ Ключи не найдены! Проверьте GOOGLE_CREDS_BASE64 в Koyeb.")
