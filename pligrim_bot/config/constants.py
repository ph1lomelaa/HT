...
# Берём JSON ключ только из переменной окружения
json_config = os.getenv("GOOGLE_CREDS_JSON")
if not json_config:
    # Если переменной нет — валимся сразу с понятной ошибкой
    raise RuntimeError(
        "❌ GOOGLE_CREDS_JSON не задан. "
        "В Koyeb в Environment variables нужно создать переменную GOOGLE_CREDS_JSON "
        "и вставить туда полный JSON сервис-аккаунта."
    )
